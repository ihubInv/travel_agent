...
// import { Plane, Send, Loader2, RotateCcw, ChevronLeft, ChevronRight, MessageSquare, Plus, Trash2, Settings, Search } from "lucide-react";
...
//         updateChatName(data.session_id, messageToSend);
//       }
...
//               ...chat, 
//               messages: { 
//                 messages: [...chat.messages.messages, userMessage, errorMessage]
//               }
//             }
//           : chat
//       ));
...
//       if (currentChatId === id) {
//         const remainingChats = chatHistory.filter(chat => chat._id !== id);
//         if (remainingChats.length > 0) {
//           await switchChat(remainingChats[0]._id, true);
//         } else {
//           await addNewChat();
//         }
//       }
...
//     ),
//     tbody: ({ node, ...props }) => (
//       <tbody className="bg-gray-900">{props.children}</tbody>
//     ),
//     tr: ({ node, ...props }) => (
...
//     ),
//     th: ({ node, ...props }) => (
//       <th className="text-left px-5 py-3 text-sm font-semibold text-gray-200 border-r border-gray-700 last:border-r-0 whitespace-nowrap">
//         {props.children}
//       </th>
//     ),
//     td: ({ node, ...props }) => (
//       <td className="text-left px-5 py-3 text-sm text-gray-300 border-r border-gray-800 last:border-r-0 whitespace-nowrap align-middle">
//         {props.children}
//       </td>
//     ),
...
//                               initial={{ opacity: 0, y: 20 }}
//                               animate={{ opacity: 1, y: 0 }}
//                               className={`flex ${msg.type === 'user' ? 'justify-end' : 'justify-start'} message-item`}
//                             >
//                               <div className={`flex gap-3 max-w-[80%] ${msg.type === 'user' ? 'flex-row-reverse' : ''}`}>
//                                 <div className={`h-8 w-8 rounded-full flex items-center justify-center ${msg.type === 'user' ? 'bg-gradient-to-r from-primary to-secondary' : 'bg-primary/10'
//                                   }`}>
//                                   {msg.type === 'user' ? '👤' : <Plane className="h-4 w-4 text-primary" />}
//                                 </div>
...
//                                       prose-pre:whitespace-pre-wrap prose-pre:break-words prose-pre:bg-gray-900 prose-pre:text-white prose-pre:rounded-lg
//                                       prose-img:rounded-lg prose-img:shadow-md prose-img:border dark:prose-img:border-gray-700"
//                                       >
//                                         <ReactMarkdown
...
//                                   {msg.type === 'bot' && suggestions && suggestions.length > 0 && (
//                                     <div className="flex flex-wrap gap-2 mt-3">
//                                       {suggestions.map((suggestion: string, index: number) => (
//                                         <button
//                                           key={index}
//                                           className="bg-indigo-100 hover:bg-indigo-200 text-indigo-800 rounded-full px-3 py-1 text-xs font-medium transition-colors border border-indigo-200"
//                                           onClick={() => {
...
export default function ChatPage() {
  ...
  const markdownComponents: Components = {
    ...
    p: ({ node, children, ...props }) => {
      // Check if the paragraph contains a pre element
      const hasPre = React.Children.toArray(children).some(
        child => React.isValidElement(child) && child.type === 'pre'
      );
      
      // If it contains a pre, render as div instead
      if (hasPre) {
        return <div className="mb-4 text-base leading-relaxed" {...props}>{children}</div>;
      }
    ...
    li: ({ node, ...props }) => <li className="mb-1">{props.children}</li>,
    table: ({ node, ...props }) => (
      <div className="overflow-x-auto rounded-xl border border-gray-700 shadow-lg my-4">
        <table className="min-w-full border-collapse text-sm">{props.children}</table>
      </div>
    ),
    thead: ({ node, ...props }) => (
    ...
    return (
      <div className="flex flex-wrap gap-2 mt-3">
        ...
        {suggestions.map((suggestion, index) => (
          <button
            key={index}
            className={`bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-full px-4 py-2 text-sm font-medium transition-colors border border-indigo-200 flex items-center gap-2 ${
