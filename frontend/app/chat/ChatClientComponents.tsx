...
export default function ChatClientComponents() {
  ...
  const getChatHistory = async (retryCount = 0): Promise<ChatSession[]> => {
    const maxRetries = 3;
    ...
    try {
      ...
      if (retryCount < maxRetries) {
        console.log(`Retrying fetch chat history (attempt ${retryCount + 1}/${maxRetries})...`);
        await new Promise(resolve => setTimeout(resolve, retryDelay));
        return getChatHistory(retryCount + 1);
      }

      // If all retries failed, show error message and return empty array
      setChatHistory([]);
      setMessages([{
        type: 'bot',
        content: "I'm having trouble connecting to the server. Please check your internet connection and try refreshing the page.",
        timestamp: new Date()
      }]);
      return [];
    }
      ...
      try {
        ...
        if (sessions.length === 0) {
          try {
            ...
            setMessages([{
              type: 'bot',
              content: "I'm having trouble creating a new chat. Please check your connection and try refreshing the page.",
              timestamp: new Date()
            }]);
          }
          return;
        ...
        setMessages([{
          type: 'bot',
          content: "I'm having trouble initializing the chat. Please check your connection and try refreshing the page.",
          timestamp: new Date()
        }]);

        // Try to create a new chat as a fallback
        try {
          const sessions = await getChatHistory();
          if (sessions.length === 0) {
            await addNewChat();
  ...
  const userFirstMessage = [
    "Search a flight from Goa to Mumbai on 16th June 2025 for 2 adults in Economy",
    "Tell me the travel advisory for Syria",
    "Book the flight with offer ID: 2",
    "Find hotels near India Gate, Delhi"
  ]
              ...
              <div className="flex-1 overflow-y-auto p-4" ref={chatBoxRef}>
                {messages.length === 0 ? (
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-center p-8">
                    ...
                    {Array.from({ length: Math.ceil(userFirstMessage.length / 2) }, (_, rowIndex) => (
  <div key={rowIndex} className="flex flex-row gap-2">
    {userFirstMessage.slice(rowIndex * 2, rowIndex * 2 + 2).map((msg, idx) => (
      <button
        key={idx}
        onClick={() => handleInitialMessageClick(msg)}
        disabled={isBotLoading}
        className="box-content size-16 border-4 p-2 rounded-lg w-full bg-gradient-to-r from-primary to-secondary hover:from-primary/90 hover:to-secondary/90 transition-all duration-300 disabled:opacity-50 cursor-pointer"
      >
        <p className="text-base font-medium text-foreground text-center text-wrap pt-4">{msg}</p>
      </button>
    ))}
  </div>
))}
